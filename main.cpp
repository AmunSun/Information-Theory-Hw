...
int main()
{
    ...
    int i=0,wrong=0;
    while(SourceDecodeResult[i]!='\0'&&original[i]!='\0'){
        i++;
        if(SourceDecodeResult[i]!=original[i])
            wrong++;
    }
    float ber=wrong/InputInfo.originLength;
    cout << "!---误比特率为---!"<<endl<<ber<<endl; ;
    i=0;wrong=0;
    while(ChannelDecodeResult[i]!='\0'&&ChannelResult[i]!='\0'){
        i++;
        if(SourceDecodeResult[i]!=original[i])
            wrong++;
    }
    int j=0;
    while(ChannelDecodeResult[i]!='\0'){
        j++;
    }
    cout << "!---误码率为---!"<< endl<<wrong/(j+1)<<endl;;
    cout << "!---通信系统仿真结束---!" << endl << endl;
...
InputType InputInformation()
{
    ...
    if(result.ChannelCodeType== 2){
        cout << "信道传输效率为："<<0.33333<<endl;
    }
    if(result.ChannelCodeType== 3){
        cout << "信道传输效率为："<<0.57143<<endl;
        
    }
    if(result.ChannelCodeType== 1){
        cout << "信道传输效率为："<<1<<endl;
